clear all
close all
video=vision.VideoFileReader('c.mp4');
...
%figure(2),imshow(im_denoise_gray)
% im_denoise_gray = medfilt2(im_gray, [1 1]);
% figure(3),imshow(im_denoise_gray)

    %% Get the Region of interest through observation
    x=[539 680 1256 89];%3rdrightdown 2ndrightup
    y=[419 417 708 716];%4thleftdown 1stleftup
    m= 720; n=1280;

    mask = poly2mask(x, y, m, n);
    %mask_img=im2double(im_denoise_gray).*mask;
    imdouble= im2double(im);
 %   figure(4),imshow(imdouble)
    ...
    im_sat = im_hsv(:,:,2);
    im_val = im_hsv(:,:,3);
%     figure(4),imshow(im_hsv);
%     figure(5),imshow(im_hue);
%     figure(6),imshow(im_sat);
%     figure(7),imshow(im_val);
